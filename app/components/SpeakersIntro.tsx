    ...
    <section className="bg-aurora-kcs px-6 py-20 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2">
          ...
          <div>
            <p className="text-xs font-semibold tracking-[0.22em] text-white/70">SPEAKER</p>
            ...
            <p className="mt-4 max-w-prose text-white/80">
              At our event, we’re thrilled to host a lineup of amazing speakers who are leaders
              and innovators in their fields. These distinguished individuals bring a wealth of
              knowledge, experience, and inspiration to our audience.
            </p>

            <ul className="mt-6 space-y-3 text-white/85">
              <li className="flex items-center gap-3">
                <span className="h-px w-8 bg-white/60" />
                <span className="font-semibold text-fuchsia-200">Keynote Speaker</span>
              </li>
              <li className="flex items-center gap-3">
            ...
            {cards.map((sp, i) => (
              ...
              >
                ...
                <div className="relative rounded-xl bg-white/5 p-3">
                  <p className="text-[11px] font-semibold leading-tight text-white/95">{sp.name}</p>
        ...
        {/* LABEL: Company Logo */}
        <p className="mb-3 text-sm font-semibold tracking-[0.18em] text-white/70">Company Logo</p>

        {/* STRIP LOGO dengan dashed separators */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-md shadow-[0_18px_60px_rgba(0,0,0,.35)]">
          <div className="flex flex-wrap items-center gap-5 md:gap-8">
            {logos.map((logo, idx) => (
              <React.Fragment key={idx}>
                {idx !== 0 && (
                  <span className="hidden h-px flex-1 border-t border-dashed border-white/30 md:block" />
                )}
                {logo.src ? (
                  <Image
                    src={logo.src}
                    ...
                    className="h-7 w-auto opacity-90 transition hover:opacity-100 md:h-8"
                  />
                ) : (
